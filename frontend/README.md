...
工作台首屏固定突出四个主操作：问知识/查数据、审文档、生成报告、识别材料；知识库、数据源、模板、任务作为支撑入口展示，避免用户一进入系统就面对大量一级菜单。审查文件上传并入合规审查，不再作为独立菜单入口；项目成员并入项目管理行内抽屉；旧 `/files` 地址重定向到 `/review`，旧 `/project/members` 地址重定向到 `/projects`。
...
| 14 OCR识别 | `/ocr` | 身份证、车牌、发票、自定义字段入口和 JSON 下载。 |
